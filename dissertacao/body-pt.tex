...
contorno obtém-se como saída (8 6 9 5), ou seja $(retr(P(5 9 6
8)))=N(8 6 9 5)$ (figura \ref{fig:operacoes-rp}). A operação de
rotação de fator 1 aplicada a este mesmo contorno P(5 9 6 8) retorna
L(9 6 8 5), ou seja, \code{(rot(P(5 9 6 8)) 1)} = L(9 6 8 5) (figura
...
%% rever conceito de classe de contorno e translação
A classe de contorno (\tr{cc})\footnote{Friedmann utiliza maiúsculas
...
\code{#p(0 3)} com a macro de leitura. Da mesma maneira, um
\texttt{contorno-duracao} com dois pontos pode ser instanciado com
\code{#d(#p(0 3) #p(1 4))} ao invés de usar \texttt{make-instance}.
...
\code{(5 9 6)}, e como contornos com duração, considerando também o
...
\code{{ \clef bass c4 d8 e d4 c }}. É possível utilizar variáveis para
...
alterado e a linha com o texto \code{43} é substituída por
\code{43 MS}. O sistema compara a versão do diretório com a versão do
