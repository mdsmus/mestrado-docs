...
objeto. Pode ser bidimensional e associar uma dimensão como altura a
outra como o tempo, ou multidimensional, associando três ou mais
dimensões, como por exemplo, altura e comprimento ao tempo. Neste
trabalho utilizo apenas contornos bidimensionais. Em Música contornos
podem ser associados a altura, densidade, ritmo, complexidade rítmica,
homogeneidade orquestral, amplitude de harmônicos, intensidade,
etc. Contornos melódicos estão relacionados com movimento de altura de
notas em função do tempo.

%% FIXME: sugestão gbertis: nota com link para referência de coerência
...
na criação musical e contribui com novas ferramentas para a área de
Composição.
...
operações de contornos melódicos. A primeira---\opus{Como é que se
  preenche um contorno melódico? Op.3}---contém um único contorno
melódico e operações de preenchimento deste contorno. A
segunda---\opus{Bobó de legumes. Op.5}---contém também um único
contorno, suas operações e concatenações. Com estes experimentos foi
possível aprofundar a prática da composição com contornos antes de
...
gráficas de operações como inversão, retrogradação e rotação de um
contorno dado. Além disso ele permite a combinação de operações e sua
concatenação. Este software ajudou no mapeamento e processamento de
contornos melódicos durante o estudo do estado de arte, na composição
dos experimentos, e na composição da obra final. Abordo o
...
Existem várias definições de contorno melódico na literatura
...
são semelhantes e em geral consideram contorno melódico como o
conjunto dos movimentos ascendentes e descendentes entre notas
adjacentes de uma melodia.
...
parâmetros como densidade de acordes e duração.

Contorno é um conjunto ordenado porque a permutação dos seus elementos
pode resultar em um contorno idêntico ou não ao seu
original. Permutação em conjuntos que não têm repetição de elementos
gera conjuntos diferentes do seu original. Permutação em conjuntos que
têm repetição de elementos pode gerar conjuntos idênticos ou não ao
seu original. Por exemplo, a rotação de fator 2 de um conjunto
original (0 1 0 1) resulta no mesmo conjunto original.
...
%% FIXME: Gbertis: nota para teorias de conjuntos
...
temporal em que as notas ocorrem na melodia. Neste exemplo, na ordem
3, 1, 2 e 0.
...
de altura entre os elementos do contorno fica clara na figura
...
ser associado ao parâmetro altura e representar as notas lá, fá,
si$\flat$ e ré (figura \ref{fig:pitches-in-time}); pode ser associado
a densidades de acordes e representar acordes de duas, uma, quatro e
sete notas (figura \ref{fig:chord-densities-in-time}); ou pode ainda
ser associado a níveis de dinâmica e representar níveis como
\music{p}, \music{ppp}, \music{mf} e \music{ff} (figura
...
A preservação de contornos e variação de intervalos entre notas é
encontrada em diferentes situações musicais. Alguns exemplos mais
comuns são as mudanças de modo em peças do tipo tema e variações, a
adequação de notas à tonalidade em respostas tonais de fugas, os
\eng{leitmotifs} e as idéias fixas \cite[p. 29]{morris87:composition}.
...
1. Dessa forma não abordei características dos contornos como
...
manipulação em um contorno ou de ação que gera um resultado, como
...
utilizados, por exemplo, em sistemas \eng{query by humming}---de busca
...
pitch or set class relations''\footnote{``contorno, na falta de outros
  sistemas abrangentes de organização de altura, representa um fator
  estrutural igual em significado a relações de alturas ou de classes
  de conjuntos''. Todas as traduções são minhas.}
\cite[p. 157]{clifford95:contour}.
...
terceiro movimento da obra \eng{Fünf Sätze für
...
principal é construído com os contornos de 3 notas\footnote{Um
  contorno de 3 elementos tem 6 possíveis permutações não ordenadas:
  (0 1 2), (0 2 1), (1 0 2), (1 2 0), (2 0 1), e (2 1 0).} (0 1 2), (2
0 1), (0 2 1) e (2 1 0) e 80\% dos fragmentos melódicos do movimento
derivam dos contornos deste tema principal.
...
\ref{fig:webern-concatenacao-1} ambos os contornos (1 2 0) e (2 0 1)
estão representados no tema principal. O contorno (2 0 1) aparece em
sua forma original e (1 2 0) como retrógrado de (0 2 1). As notas e
intervalos do exemplo são diferentes das utilizadas no tema. Por
exemplo, no tema o contorno (0 2 1) tem intervalos de meio tom entre
as notas. No exemplo da figura \ref{fig:webern-concatenacao-1} o
retrógrado deste contorno tem intervalos de terça menor e nona
menor. Na figura \ref{fig:webern-concatenacao-2} ocorre uma repetição
do contorno (2 1 0) e o contorno (1 0 2) obtido por retrogradação do
contorno (2 0 1). Como foi dito estas operações são explicadas na
seção \ref{sec:teor-de-cont}.
...
combinar muitas operações, como retrógrado da inversão da rotação.
...
operações como $retr(inv(rot(X(1\;2\;3))\;2))$. Esta
...
contorno obtém-se como saída N(8 6 9 5), ou seja
...
\ref{fig:operacoes-simples}. Neste contorno o retrógrado da inversão
(figura \ref{fig:operacoes-rip}) é igual à rotação de fator 2 (figura
\ref{fig:operacoes-rr2}).
...
A ordem de um contorno é calculada através da soma do maior valor
numérico dos seus elementos com 1. Um dado contorno P(5 9 6 8), por
exemplo, tem ordem 10 $(9+1)$. O espaço de contorno ao qual P pertence
contém pelo menos os elementos de valores numéricos menor que 9, ou
seja, além de 5, 9, 6 e 8, os elementos 0, 1, 2, 3, 4 e 7. Porém é
impossível determinar se este espaço de contorno tem outros elementos
maiores do que 9.  Então, na falta de determinação de um conjunto
universo ao qual o contorno faz parte, o contorno pode ser confinado
em um sistema de base $n+1$, onde $n$ é o maior valor numérico.
...
com operação de comparação, matriz de comparação, forma prima e classe
de contorno.
...
A classe de contornos equivalentes é importante para a verificação de
similaridade entre contornos. Dois ou mais contornos são considerados
equivalentes quando geram uma mesma matriz de comparação, ou seja,
quando mantêm a mesma estrutura de valores entre seus elementos. Dessa
forma, dado um contorno P(5 9 6 8), são considerados seus equivalentes
os contornos (1 5 2 3), (0 10 4 7), (0 3 1 2) e muitos outros, pois
todos eles pertencem à classe (0 3 1 2). Também são considerados
contornos de classe equivalente aqueles relacionados por operações de
translação, retrógrado e inversão.
...
O vetor de intervalos de contornos descreve a freqüência de cada tipo
...
programa como
%% FIXME: espinheira: tirar seção sobre lilypond. usar nota
...
No \goiaba{} contornos são representados simbolicamente de duas
maneiras: como contornos simples, considerando apenas os valores dos
elementos de contorno \code{(5 9 6)}, e como contornos com duração,
considerando também o local no tempo onde ocorre cada elemento do
contorno: \code{((0 5)(1 9)(2 6))}. Apesar de ter uma codificação
...
do tipo \texttt{ponto} pode-se fazer normalmente \code{(make-instance
  'ponto :x 0 :y 3)} ou simplesmente \code{\#p(0 3)} com a macro de
leitura. Da mesma maneira, um \texttt{contorno-duracao} com dois
pontos pode ser instanciado com \code{\#d(\#p(0 3) \#p(1 4))} ao invés
...
Lisp. Desse modo um método como \texttt{rotacionar} se comporta de
...
inversão, retrogradação, rotação e expansão de intervalos. Além disso
operações definidas na literatura também estão implementadas, como
redução de contornos \cite{adams76:melodic}, classe de contorno, série
de contornos adjacentes, vetor de séries de contornos adjacentes,
intervalo de contorno, vetor de intervalo de contorno, vetor de classe
de contorno I e II \cite{friedmann85:methodology} e matriz de
comparação \cite{morris93:directions}.
...
original, retrogradação, inversão, rotação e ampliação de altura
(figura \ref{fig:operacoes}). Neste exemplo defino o contorno
\code{Z}, o arquivo no qual o gráfico será criado, as dimensões do
...
(let ((Z #s(0 5 3 4 1 3)))
  (plot-page "contornos.pdf"
    (plot-contorno-full 50 500
       ...
       (aumentar-altura Z 2) "aumentar-altura" :green
       (rotacionar Z 1) "rotação" :darkcyan)))
\end{verbatim}
\doublespacing
...
Para o futuro pretendo implementar a geração de contornos a partir de
partituras musicais em formato Lilypond e vice-versa, e implementar
...
internet\footnote{Disponível em \url{http://genos.mus.br/goiaba/}.}.
...
um esquema gráfico deste processo. Neste exemplo o arquivo \code{foo}
é alterado e a linha com o texto \code{43} é substituída por \code{43
  MS}. O sistema compara a versão do diretório com a versão do
...
\begin{table}
  ...
  \centering
  \includegraphics[angle=90,scale=.76]{planejamento-inicial}
...
obtida com a variação de registro (representada na figura
\ref{fig:registro-secao-2}) e com o estreitamento e alargamento das
entradas instrumentais. A variação de registro está associada à
...
\begin{figure}
  ...
  \caption{Representação do registro na seção 2 em \eng{piano roll}}
  \label{fig:registro-secao-2}
\end{figure}
...
dos toques da capoeira\footnote{A utilização deste elemento foi apenas
  pontual. Por este motivo não aprofundo em maiores explicações sobre
  a capoeira e seus toques.}. O ritmo do ostinato é aproveitado para a
construção da estrutura de duas vozes (vide seção
...
\begin{figure}
  \centering
  \subfloat[Textura de notas curtas e silêncios (som real)]{
...
\begin{figure}
  ...
  \caption{Apresentação da estrutura de duas vozes no clarinete e
    trompa (ambos em som real) c. 161--166}
...
\begin{figure}
  ...
  \subfloat[Rotação 3 e retrógrado: (4 3 5 0 2 1)]{
    \includegraphics{c-435021}
    \label{fig:205341}  
  }
...
\begin{figure}
  \centering
  \includegraphics[scale=5.5]{oboe-solo-secao-5}
...
musicais como andamento, textura e quantidade de instrumentos tocando
simultaneamente.
...
O terceiro trabalho é a incorporação da medida do tempo no estudo de
contornos. Este trabalho possibilita o estudo de características tais
como inclinações entre os elementos de contorno.

O quarto trabalho é a composição de estudos com instrumentos mais
...
Finalmente o quinto trabalho é a expansão do software \goiaba{}, com a
