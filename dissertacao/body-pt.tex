...
%% marcos: o que mais deve estar na introdução?
...
%% rever frase. substituir 'colaborar'
%% marcos: não sei bem o que colocar
Este estudo pode contribuir tanto com o estado de arte de contornos
quanto com a área de Composição.
...
Computação musical, especificamente, \eng{Music Information
  ...
  humming} --- de busca em bases de dados a partir de melodias
...
Clifford afirma que \citacaoinline{contour, in absence of other
  pervasive systems of pitch organization, represents a structural
  factor equal in significance to pitch or set class
  relations}{contorno, na falta de outros sistemas ocupantes de
  organização de altura, representa um fator estrutural igual em
  significado a relações de alturas ou de classes de
  conjuntos}{p. 157}{clifford95:contour}
...
%% usar termos cseg, c-space, etc em português

%% em algum lugar colocar tabela de classes de segmentos de contorno
...
\begin{figure}
  \centering
  \subfloat[melodias com cseg P(5 9 6 8)]{
    \includegraphics[scale=.9]{ly-5968}
    \label{fig:ly-cseg-5968}
  }
  \subfloat[melodias com cseg Q(5 7 6 8)]{
    \includegraphics[scale=.9]{ly-5768}
    \label{fig:ly-cseg-5768}
  }

  \subfloat[melodias com cseg R(3 0 5 1)]{
...
é chamado de segmento de contorno (\tr{cseg})\footnote{Trata-se de uma
  ...
  segmento de reta.}. Para uma simplificação terminológica neste
trabalho nos referimos a segmento de contorno com o termo genérico
``contorno''. Estes contornos podem conter elementos contíguos ou não
contíguos do espaço de contorno. Subconjuntos de contornos são
chamados \tr{csubseg}. A figura \ref{fig:c-space} contém três exemplos
de um mesmo espaço de contorno de 10 alturas de contorno enumeradas do
mais grave (0) para o mais agudo (9). Na figura \ref{fig:c-space5968}
o espaço de contorno contém o contorno P, com as alturas de contorno
5, 9, 6 e 8. Na figura \ref{fig:c-space7420} o espaço de contorno
contém o contorno M, com alturas de contorno 7, 4, 2 e 0. Ainda nesta
figura o contorno M contém o \tr{csubseg} O, com alturas de contorno 4
e 2. Por último, na figura \ref{fig:c-space564} o espaço de contorno
contém o contorno N, com as alturas de contorno não adjacentes 5, 6 e
4.
...
%% confirmar procedimento para expansão de intervalos
...
%% reescrever para tirar nota
%% marcos: top colocou esta nota, mas estamos entendendo que matrizes
%% são operações. posso manter o original?
...
A \tr{COM-matrix} é uma matriz bidimensional que compara um contorno
com ele próprio. Esta matriz mostra os resultados da função de
comparação \tr{COM} para todas as alturas de contorno de um
contorno. A tabela \ref{fig:matriz-5968} contém a matriz de
comparação de um contorno P(5 9 6 8). As \tr{COM-matrix} dos exemplos
já vistos na figura \ref{fig:graficos-cseg} podem ser vistos na tabela
...
%% marcos: eu dou exemplos de int logo abaixo. o que quer dizer com
%% apontar valores no exemplo?
...
(\tr{CAS}) \footnote{Em teorias de contornos não há consenso em
  relação a terminologia. Por isso há idéias semelhantes com nomes
  diferentes, como INT$_1$ e \tr{CAS} \cite{friedmann87:response}.}.
...
contorno. Uma \tr{CC} engloba todos os contornos considerados
equivalentes. Dois ou mais contornos são considerados equivalentes
quando geram uma mesma \tr{COM-matrix}, ou seja, quando mantêm a mesma
estrutura de registro entre suas notas. Dessa forma, dado um contorno
P(5 9 6 8), são considerados seus equivalentes os contornos (1 5 2 3),
(0 10 4 7), (0 3 1 2) e muitos outros. Todos eles têm (0 3 1 2) como
forma normal e \tr{CC}.

Os Intervalos de Contorno (\tr{CI}) representam as relações entre
alturas de contorno de uma \tr{CC} e podem ser entendidos de duas
...
1 2), o \tr{CI} entre $P_1$ e $P_2$ para Friedmann é -2, e para
Morris, ``$-$''.

O Vetor de Intervalos de Contornos (\tr{CIA}) descreve a freqüência de
cada tipo de \tr{CI} em uma \tr{CC}. Por exemplo, o contorno P(0 3 1
2) tem \tr{CIA} (2,1,1/1,1,0). Os dígitos da esquerda da barra
...
Morris define os conceitos de espaço de alturas (\tr{p-space}) e
espaço de contorno (\tr{c-space}) \cite{morris87:composition}. Em
...
A Matriz de Comparação (\tr{com-matrix}) representa o total de
...
forma INT$_1$ representa as diferenças de altura entre \tr{cps}
adjacentes, INT$_2$ representa as diferenças de altura entre as
alturas de contorno separadas não adjacentes separados por um elemento
e assim por diante \cite[p. 231]{marvin.ea87:relating}. Dado um
contorno (4 3 5 6), INT$_1$ = (- + +) representa as diferenças entre 4
e 3, 3 e 5, e 5 e 6. INT$_2$ = (+ +) representa as diferenças entre 4
e 5, e 3 e 6.
...
mesma \tr{com-matrix}, e a segunda --- \tr{csegclass} --- formada por
todos os contornos relacionados por operações de identidade,
translação, retrogradação, inversão e retrogradação da inversão.

A operação de inversão é obtida pela subtração de todos os \tr{cps}
por $(n-1)$. Para um dado contorno $P$, a operação de identidade é
identificada como $IP$. A retrogradação de um contorno $P$
(representado por $RP$), bem como de sua inversão ($IP$) são obtidas
colocando os \tr{cps} em ordem reversa
...
\item se a subtração de $(n-1)$ pelo último \tr{cps} é menor que o
  primeiro \tr{cps}, inverte-se o contorno;
\item se o último \tr{cps} é menor que o primeiro \tr{cps}
  retrograda-se o contorno.
\end{enumerate}
...
criou duas ferramentas principais: \tr{cas} e \tr{cc}
...
%% fazer introdução sobre ferramentas
...
O Git---\eng{Fast Version Control System}
...
todo o trabalho. É possível baixar o repositório de todo este trabalho
em \url{git.genos.mus.br/marcos-mestrado.git}. Usamos controle de
versão no gerenciamento do desenvolvimento do Goiaba, nos experimentos
composicionais, na obra final, no projeto e na escrita desta
dissertação.
...
%% bifonia: paulo lima sugeriu citar que encontrei uso deste termo em
%% widmer
...
%% usar nome das seções ao invés de primeira, segunda
