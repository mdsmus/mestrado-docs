...
%% começar falando o que é contorno
...
cada uma delas relacionada com o objetivo do trabalhos de seu autor.
%% dar exemplo
...
da música. Na área da Etnomusicologia contornos são usados para
representar e classificar melodias populares
\cite{adams76:melodic}. Na área da Computação musical,
especificamente, \eng{Music Information Retrieval}, contornos são
utilizados em sistemas \eng{query by humming} --- de busca em bases de
dados a partir de melodias cantadas \cite{ghias.ea95:query}.

%% remover nome da seção ou criar uma nova seção neste capítulo
...
obra musical com base em combinações de operações de contorno e a
...
%% explicar melhor de modo que um leigo entenda
...
%% colocar inline e indicar que a tradução é minha
...
\note{período fragmentado}.
%% dizer porque as melodias não são semelhantes se não vemos o
%% contorno
...
\ref{fig:ly-cseg-5968}, cujo contorno está representado graficamente
na figura \ref{fig:cseg-5968}. Outros exemplos de melodias de
contornos semelhantes podem ser vistos nas figuras
\ref{fig:melodias-cseg} e \ref{fig:graficos-cseg}.
...
segmento de contorno (\tr{cseg})\footnote{Trata-se de uma idéia
  semelhante à da geometria, de reta e segmento de reta, onde o
  \tr{c-space} seria análogo à reta, e o \tr{cseg} ao segmento de
  reta.}. Estes \tr{cseg} podem conter elementos contíguos ou não
contíguos do \tr{c-space}. Subconjuntos de \tr{cseg} são chamados
\tr{csubseg}. A figura \ref{fig:c-space} contém três exemplos de um
mesmo \tr{c-space} de 10 \tr{c-pitch} enumerados do mais grave (0)
para o mais agudo (9). Na figura \ref{fig:c-space5968} o \tr{c-space}
contém o \tr{cseg} P, com os \tr{c-pitch} 5, 9, 6 e 8. Na figura
\ref{fig:c-space7420} o \tr{c-space} contém o \tr{cseg} M, com
\tr{c-pitch} 7, 4, 2 e 0. Ainda nesta figura o \tr{cseg} M contém o
\tr{csubseg} O, com \tr{c-pitch} 4 e 2. Por último, na figura
\ref{fig:c-space564} o \tr{c-space} contém o \tr{cseg} N, com os
\tr{c-pitch} não adjacentes 5, 6 e 4.
...
multiplicação dos intervalos entre cada \tr{c-pitch} por um dado
...
\tr{cseg} P(5 9 6 8). As \tr{COM-matrix} dos exemplos já vistos na
...
entre suas notas. Dessa forma, dado um \tr{cseg} P(5 9 6 8), são
considerados seus equivalentes os \tr{cseg} (1 5 2 3), (0 10 4 7), (0
...
P(0 3 1 2), o \tr{CI} entre $P_1$ e $P_2$ para
...
estrutura é o contorno principal da peça \contpr{} (figura
...
mesma melodia (figura \ref{fig:melodia-inicial}). Esta repetição é
...
(figura \ref{fig:transicao-3-4}). A quarta seção é um fugato com
...
fagote (figura \ref{fig:ostinato-fagote}). Este ostinato é inspirado
em um dos toques da capoeira\footnote{A utilização deste elemento foi
  apenas pontual. Por este motivo não aprofundaremos em maiores
  explicações sobre a capoeira e seus toques.}. O ritmo do ostinato é
...
separados entre si por silêncios (figura
...
uma repetição do que ocorre na quarta seção. A linha do clarinete
representa o ritmo e acentuação da terceira seção. A linha da trompa
corresponde ao solo do fagote do início da peça. As linhas do oboé e
flauta representam a bifonia apresentada na quinta seção. Finalmente o
material apresentado pela flauta e oboé a partir do compasso 239
representa o sujeito do fugato da quarta seção. As relações de
...
sonoridade octatônica. A terceira seção da peça é inteiramente
...
(figura \ref{fig:bifonia-grave}). Aparece inicialmente na região
...
da sexta seção. O sujeito do fugato (figura \ref{fig:sujeito-fugato})
é constituído por dois segmentos elididos. O primeiro segmento é o
...
linhas dos instrumentos (figura \ref{fig:contorno-notas-longas},
...
a quinta seção (figura \ref{fig:ostinato-fagote},
