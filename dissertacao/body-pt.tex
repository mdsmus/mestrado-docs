...
%% dúvida: o que mais deve estar na introdução?


%% kroger: 
%% * "organização da dissertação" no final do capitulo
%% * deixar mais claro o problema que voce vai resolver. veja a intro
%% da minha tese
...
%% dúvida: o que coloco no lugar de contribuir/colaborar?

%% kroger: nesse contexto eu acho que "elevar o estado de arte" é
%% melhor. talvez seja melhor ser mais especifico: elevar o estado de
%% arte da teoria de contornos e contribuir com novas ferramentas para
%% a area de composicao
...
%% dúvida: o que não acha bom neste parágrafo?

%% kroger: eu nao acho que exista essa relacao de causa e consequencia
%% (apenas) "vamos criar uma teoria de contorno porque os ouvintes
%% ouvem melhor contornos", mesmo que marvin diga isso, parece que
%% *todas* as teorias de contornos foram feitas por isso, o que os
%% outros autores nao afirmam
...
%% dúvida: você pediu para colocar referência na frase acima. a
%% referência é a mesma da frase seguinte. como resolvo?

%% kroger: talvez dando um merge nas duas frases
...
%% dúvida: top colocou esta nota, mas estamos entendendo que matrizes
...
contorno\note{fica parecendo que matrizes de comparação e classes de
  contorno são operações, acho bom mudar a vírgula de lugar ou algo
  assim}.
...
As diagonais superiores à diagonal principal zero da matriz de
comparação são chamadas de INT$_n$, onde $n$ é o número da diagonal: 1
para a superior mais próxima da diagonal zero, 2 para a seguinte, 3
para a posterior e assim por diante.
%% dúvida: você pediu para apontar valores no exemplo. eu dou exemplos
%% de int logo abaixo. o que quer?

%% kroger: que voce faça uma ligação dos exemplos no texto com os
%% exemplos graficos para o leitor visualizar
...
(\tr{CAS})\footnote{Em teorias de contornos não há consenso em relação
  ...
  alturas de contorno adjacentes.}.
...
mesma matriz de comparação, e a segunda, chamada de \tr{csegclass}, é
...
A operação de inversão é obtida pela subtração de todos as alturas de
...
%% substituir bifonia por algo como 'estrutura de duas vozes'
