...
A obra contém duas estruturas musicais das quais todo o material
composicional é derivado. A primeira estrutura é uma bifonia
representada pela figura \ref{fig:bifonia} e a segunda é o motivo
principal representado pela figura \ref{fig:motivo-principal}. Este
motivo é derivado por meio de alternância entre as notas das duas
vozes da bifonia.
...
O motivo principal possui um contorno de classe \contpr{}. Este
contorno e as combinações das suas operações associadas são utilizados
na composição de toda a obra.
...
figura \ref{fig:melodia-inicial}). Esta melodia pode ser dividida em
duas partes: a primeira derivada das três notas iniciais do motivo
principal; e a segunda como transposição da primeira, com expansão de
intervalos e preservação do contorno.
...
A quarta seção da obra é um fugato com sujeito e contra-sujeito
derivados do contorno principal e de operações. O sujeito (vide figura
\ref{fig:sujeito-fugato}) é constituído por dois segmentos: o contorno
principal (5 3 4 1 2 0) e sua rotação de fator 3 (1 2 0 5 3 4). O
segundo segmento é iniciado na nota $G$, última nota do segmento
anterior.
