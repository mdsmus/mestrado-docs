...
Q é $\frac{5}{6}$, ou CSIM(P,Q)$=0,83$. Entre P e R há apenas um
elemento semelhante entre 6 possíveis. Assim, CSIM(P,R)$=0,16$. A
tabela \ref{tab:similaridade-contornos} mostra os valores de CSIM
...
valor absoluto. \cite{friedmann85:methodology}. Dessa forma
consideremos que no contorno P(0 3 1 2) há 2 intervalos de valor +1, 1
intervalo de valor +2 e outro intervalo de valor +3. Além disso há um
intervalo de valor -1, outro intervalo de valor -2 e nenhum intervalo
de valor -3. Os Vetores de classe de contorno (\tr{CCVI} e
\tr{CCVII}), de dois dígitos cada um, refletem os movimentos
ascendentes e descendentes de um contorno e são calculados a partir
dos intervalos de contorno ascendentes e descendentes.
...
As teorias de contornos aqui relacionadas têm sido expandidas não
apenas pela implementação de novos conceitos e operações, mas pela
crítica que seus autores fazem aos trabalhos de outros teóricos. Uma
vez que essas teorias têm sido criticadas na literatura acadêmica
\cite{clifford95:contour,beard03:contour}, o objetivo desta revisão do
estado de arte de teorias de contornos deixa de ser a realização de
uma crítica da funcionalidade ou significado musical de cada conceito
ou operação de contorno, mas sim a reunião de operações que podem ter
suas funcionalidades testadas em aplicações na área de Composição
Musical.
...
conectividade podemos desenvolver um software mais completo sem
precisarmos necessariamente desenvolver bibliotecas ou programas já
existentes.
...
O \goiaba{}\footnote{Assim como ocorreu com o nome da empresa de
  ...
  Goiaba.} é desenvolvido na linguagem Common Lisp\footnote{Disponível
  em \url{cliki.net/CLiki}}, com o compilador SBCL\footnote{Disponível
  em \url{sbcl.org}} e com metodologia \eng{bottom-up}.

A metodologia \eng{bottom-up} está relacionada à direção do
...
%% falar sobre common lisp

Lisp é uma família de linguagens de programação que dispõe de vários
dialetos como Common Lisp e Scheme. Em Lisp, código-fonte e estrutura
de dados são criados com listas em uma sintaxe com
parênteses. Programas em Lisp podem manipular código-fonte e estrutura
de dados de forma a possibilitar a criação de novas sintaxes ou
linguagens de domínio específico \cite{kroger07:programacao}. Lisp é
descrita como uma linguagem extensível, interativa e programável
\cite{foderaro91:lisp,graham94:lisp,seibel05:practical,shapiro92:common}.

Lisp se caracteriza por um grande número de vantagens integradas como
\citacaoinline{dynamic storage allocation and garbage collection,
  ...
  lists, an interactive environment, and so on}{alocação dinâmica de
  armazenzamento e alocação do lixo, escrita em tempo de execução,
  funções como objetos, um parseador \eng{built-in} que gera listas,
  um compilador que aceita programas expressos como listas, um
  ambiente interativo e assim por diante}{p. 8}{graham94:lisp} Lisp é
utilizada comercialmente no desenvolvimento de softwares como
Emacs\footnote{Disponível em \url{gnu.org/software/emacs}.},
AutoCAD\footnote{Software proprietário. Disponível em
  \url{www.autodesk.com}.} e Interleaf\footnote{Software
  proprietário. Disponível em \url{www.broadvision.com}.}. O Lilypond
tem parte de seu código desenvolvido em Scheme.

%% falar sobre sbcl, emacs e slime
...
Neste trabalho além de desenvolvermos o Goiaba utilizamos o Lilypond
tanto para gerar partituras através do Goiaba quanto para escrever a
obra \obra{}. Usamos também o sistema de controle de versão Git que
ajudou no desenvolvimento do Goiaba, na composição da obra e na
comunicação de dados com o orientador.
...
de versão bastante poderoso e veloz. Como outros sistemas modernos,
permite o desenvolvimento distribuído, em que cada desenvolvedor pode
ter uma cópia local do repositório central, que fica acessível via
