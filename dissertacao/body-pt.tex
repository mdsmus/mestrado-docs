...
motivo $\alpha$, o principal da obra, representado pela figura
\ref{fig:motivo-alpha}. Este motivo é derivado por meio de
...
$\alpha$ (figura \ref{fig:motivo-alpha}) aparece simultaneamente
...
O motivo $\alpha$ possui um contorno de classe \contpr{}, representada
graficamente na figura \ref{fig:534120}. Este contorno e as
combinações das suas operações estão presentes em toda a obra.
...
$\alpha$, e o segundo como transposição do segmento anterior com
...
cada uma o motivo $\alpha$ modificado por operações de rotação. Os
grupos de notas curtas não englobam necessariamente todas as notas do
motivo modificado. Por vezes o silêncio interrompe o motivo, como nos
na linha da flauta, nos compassos 177--178 por exemplo (vide figura
\ref{fig:grupos-separados-silencio}).
...
A linha da flauta contém o motivo $\alpha$ em sua forma original. A
...
do motivo $\alpha$, porém com transposição da segunda metade do
...
rotação de fator 4 do motivo $\alpha$ e expansão semelhante ao do
clarinete. Na figura \ref{fig:notas-curtas-madeiras} estão exibidas
estas variações do motivo $\alpha$ em cada elemento, na tabela
