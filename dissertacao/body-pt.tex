...
notação \code{retr(X(1 2 3))}. O resultado desta operação é o contorno
Y(3 2 1). Esta notação ajuda na visualização de combinações maiores de
operações como \code{transp(retr(inv(rot(X(1 2 3)) 2)) 3)}. Esta
...
8) retorna L(9 6 8 5), ou seja, \code{rot(P(5 9 6 8) 1)} $\Rightarrow$
...
A operação de comparação \code{com(a,b)}\footnote{Operações como a
...
\code{com(P$_0$,P$_1$)} é o sinal ``$+$'', o de
\code{com(P$_1$,P$_2$)} é ``$-$'', e o valor de
\code{com(P$_3$,P$_0$)} é ``$+$''. Esta medida de comparação pode ser
...
A função de similaridade (\code{csim(X,Y)}) mede numericamente o grau
...
\ref{tab:similaridade-contornos} mostra os valores de \code{csim(X,Y)}
...
clarinete nos compassos 157--172, na construção do material ascendente
...
3). A primeira nota, Mi5, ausente no exemplo, está elidida com a
última nota do sujeito. O segundo segmento contém uma rotação de fator
4 do retrógrado do contorno principal (3 5 0 2 1 4), e também está
elidida ao segmento anterior. Finalmente o terceiro segmento contém
uma rotação de fator 3 do retrógrado do contorno principal (4 3 5 0 2
1). Ao contrário dos outros segmentos, ele não está elidido ao
segmento anterior. Pode-se observar que o contra-sujeito incorpora
ainda uma sistemática nas suas operações de contorno. A cada segmento
o fator de rotação diminui em um ponto.
...
contornos ocorre na linha do oboé nos compassos 149--153 (figura
\ref{fig:oboe-solo-secao-5}). O motivo $\alpha$ tem uma rotação de
fator 3 gerando as notas adjacentes Sol-Si$\flat$-Lá, e logo após, no
compasso 152, Dó$\sharp$-Si$\sharp$-Ré$\sharp$. Entre estes dois
...
151 e 152. Como já foi dito, este motivo é uma retrogradação do motivo
...
o quinteto completo e um quarteto nesta ordem: fagote (1),
fagote+clarinete+oboé (3), clarinete+flauta (2), todos (5) e quarteto
sem fagote (4).
...
Este trabalho contribui de forma significativa com a ampliação do
estado de arte de teorias de contorno incorporando possibilidades de
aplicações de operações de contorno na composição musical, uma vez que
há carência deste tópico na literatura.
