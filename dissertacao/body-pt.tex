...
os registros dos elementos. O \tr{c-space} pode ser entendido como um
grande conjunto de alturas de contorno (\tr{c-pitch}).

Cada conjunto de alturas de contorno contido em um \tr{c-space} é
chamado de segmento de contorno (\tr{cseg})\footnote{Trata-se de uma
  ...
  reta.}. Para uma simplificação terminológica neste trabalho nos
referimos a segmento de contorno com o termo genérico
``contorno''. Estes \tr{cseg} podem conter elementos contíguos ou não
contíguos do \tr{c-space}. Subconjuntos de \tr{cseg} são chamados
\tr{csubseg}. A figura \ref{fig:c-space} contém três exemplos de um
mesmo \tr{c-space} de 10 alturas de contorno enumeradas do mais grave
(0) para o mais agudo (9). Na figura \ref{fig:c-space5968} o
\tr{c-space} contém o \tr{cseg} P, com as alturas de contorno 5, 9, 6
e 8. Na figura \ref{fig:c-space7420} o \tr{c-space} contém o \tr{cseg}
M, com alturas de contorno 7, 4, 2 e 0. Ainda nesta figura o \tr{cseg}
M contém o \tr{csubseg} O, com alturas de contorno 4 e 2. Por último,
na figura \ref{fig:c-space564} o \tr{c-space} contém o \tr{cseg} N,
com as alturas de contorno não adjacentes 5, 6 e 4.
...
IP$_n=(q-1-$P$_n)$. Portanto, dado um \tr{cseg} P(5 9 6 8), de ordem
...
\tr{cseg}. A tabela \ref{fig:matriz-5968} contém a matriz de
comparação de um \tr{cseg} P(5 9 6 8). As \tr{COM-matrix} dos exemplos
...
adjacentes do \tr{cseg}. Em P(5 9 6 8), por exemplo, INT$_1=(+ - +)$,
...
\tr{cseg}. Uma \tr{CC} engloba todos os contornos considerados
...
estrutura de registro entre suas notas. Dessa forma, dado um \tr{cseg}
P(5 9 6 8), são considerados seus equivalentes os \tr{cseg} (1 5 2 3),
...
\cite{morris93:directions}. Por exemplo, para um mesmo \tr{cseg} P(0 3
1 2), o \tr{CI} entre $P_1$ e $P_2$ para Friedmann é -2, e para
Morris, ``$-$''.

O Vetor de Intervalos de Contornos (\tr{CIA}) descreve a freqüência de
cada tipo de \tr{CI} em uma \tr{CC}. Por exemplo, o \tr{cseg} P(0 3 1
