...
De acordo com estas definições compreende-se que o fragmento da 5ª
Sinfonia de Beethoven, por exemplo (figura
\ref{fig:melodia-5a-sinfonia}), tem contorno descendente
(Sol-Mi$\flat$), ascendente (Mi$\flat$-Fá) e descendente (Fá-Ré) e que
...
parâmetros como densidade de acordes e duração. Contorno é por
definição ordenado porque a permutação de seus elementos resulta em um
novo contorno. 
...
Tendo em vista a definição de contorno de Morris, posso afirmar que o
...
grave que a primeira, o que fica claro na figura \ref{fig:c-3120},
decorrente da definição de Morris.
...
como inclinação dos segmentos entre dois pontos, embora existam
estudos na literatura abordando este aspecto
\cite{beard03:contour}. Percebi ao longo do processo que incluir
mensurar o tempo neste estudo demandaria um prazo maior do que o
...
também são detalhadas na seção \ref{sec:teor-de-cont}.
...
A análise de obras tonais a partir de contornos tonais já se mostrou
eficiente, embora as teorias de contorno não tenham sido desenvolvidas
para este tipo de obra. Existem análises bem sucedidas de obras de
Arnold Schönberg \cite{friedmann85:methodology}, Anton Webern
\cite{clifford95:contour}, Luigi Dallapicolla
\cite{marvin88:generalized} e Wolfgang Amadeus Mozart
\cite{beard03:contour} sob a ótica das teorias de contornos.


Um contorno pode ser interpretado como altura (desconsiderando seu
valor absoluto), dinâmica ou densidade de acordes no tempo
\cite[p. 206]{morris93:directions}
\cite[p. 22]{clifford95:contour}. Um dado contorno (4 3 5 6), por
exemplo, pode ser associado ao parâmetro altura com as notas Lá, Fá,
...
\cite[p. 159]{clifford95:contour}. Esta transformação ocorre, por
exemplo, no terceiro movimento da obra \eng{Fünf Sätze für
  Streichquartett}\footnote{Cinco movimentos para quarteto de cordas.}
Op.5.
%% inserir texto e comentário em figura
...
graficamente na figura \ref{fig:cseg-0312}\footnote{Há evidências
  ...
  \cite[p. 69]{marvin88:generalized}.}. Este contorno é
representado simbolicamente por P(0 3 1 2), onde P é o seu nome, 0 é o
elemento mais grave, 1 é o segundo elemento mais grave, 2 é o terceiro
elemento mais grave e 3 o elemento mais agudo. Estes números são
apresentados na ordem temporal em que ocorrem na melodia: primeiro o
ponto mais grave 0, em seguida o mais agudo 3, e assim por diante. As
semelhanças entre estas melodias da figura \ref{fig:ly-0312} são
...
(\tr{c-space ou contour-space}) é uma abstração de espaço musical que
consiste em elementos organizados do grave para o agudo e que
desconsidera a altura precisa e os intervalos exatos entre estes
elementos. Leva-se em conta apenas a relação entre as alturas dos
elementos. O espaço de contorno pode ser entendido como um grande
conjunto de alturas de contorno (\tr{c-pitch ou contour-pitch}).
...
%% FIXME: reescrever expandindo a idéia de altura
A operação de comparação \tr{COM(a,b)} retorna a diferença de altura
entre dois elementos de contorno $a$ e $b$ considerando se um elemento
é mais agudo, mais grave ou de mesma altura que outro. O resultado da
operação de comparação é ``$+$'' se $b$ é maior que $a$; ``$-$'' se
$b$ é que $a$; e ``$0$'' se $b$ é igual a $a$. Por exemplo, no
...
%% FIXME: reescrever expandindo além de melodia
