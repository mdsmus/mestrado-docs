...
%% mover este parágrafo para uma seção sobre gestual na obra
A melodia do início da peça é repetida ao longo da primeira seção de
maneira alternada entre os instrumentos. Ainda nesta seção o motivo
principal (figura \ref{fig:motivo-principal}) aparece simultaneamente
formando uma textura contrapontística de duas vozes. O progressivo
aumento da densidade e do nível de dinâmica colaboram com o desenho
gestual da seção, que culmina no acorde do compasso 36.
...
sujeito (vide figura \ref{fig:sujeito-fugato})
...
\ref{fig:motivo-principal}) dá origem aos motivos de três notas (vide
figura \ref{fig:motivo-3-notas}) e de semicolcheias (vide figura
\ref{fig:motivo-semicolcheias}). Além destes há a presença de um
motivo unicamente rítmico (vide figura \ref{fig:motivo-ritmico}).
...
\begin{figure}
  ...
  \subfloat[Motivo rítmico]{
    \includegraphics{motivo-ritmico}
    \label{fig:motivo-ritmico}
  }
...
O objetivo principal da composição da obra \obra{} foi o uso de
combinações de operações de contorno para a criação
musical. Utilizamos um grupo com algumas das operações de contorno
disponíveis e as combinamos para criar o material musical da
obra. Trabalhamos com dois subconjuntos de contornos, operações de
$INT_1$, inversão, rotações e de preenchimento de
contornos. Combinamos estas operações desta forma:

\begin{itemize}
\item Subconjunto com expansão e transposição (fagote no início)
\item Preenchimento com expansão (oboé quinta seção)
\item Rotação com expansão (sujeito do fugato e sexta seção)
\item Concatenação de contornos (sujeito do fugato e movimentos
  ascendentes na sexta seção)
\item Rotação com retrogradação (sujeito do fugato)
\end{itemize}
...
melodia (vide figura \ref{fig:melodia-inicial}) construída com o
subconjunto do contorno principal (2 0 1). A melodia tem dois
segmentos, sendo o primeiro derivado das três notas iniciais do motivo
principal, e o segundo como transposição do segmento anterior com
operações de expansão e transposição. Em ambos os segmentos há
preservação do contorno (2 0 1). Esta melodia está presente em toda a
primeira seção da obra, nos compassos 9--14 no fagote e oboé, nos
...
primeiro segmento é o próprio motivo principal, por consequüencia, é
...
nos compassos 149--157 (vide figura \ref{fig:anacruze-oboe}). O motivo
de semicolcheias (vide seção \ref{sec:uso-de-motivos} e figura
\ref{fig:motivo-semicolcheias}) é uma retrogradação do motivo
principal da peça. Neste trecho este motivo aparece três vezes: na
primeira vez em sua forma original, na segunda vez com uma expansão e
intervalo de segunda para intervalo de terça, e na terça com uma
...
principal tem uma rotação de fator 3 gerando as notas adjacentes
Sol-Si$\flat$-Lá, e logo após, no compasso 152,
Dó$\sharp$-Si$\sharp$-Ré$\sharp$. Entre estes dois grupos de notas há
uma expansão de intervalos do motivo de semicolcheias, iniciado em
Dó$\sharp$3 e concluído em Dó$\sharp$4. Como já foi dito, este motivo
é uma retrogradação do motivo principal. Entendemos que este motivo
está interpolado com o contorno iniciado em Sol.
...
Sol5---estão em diferentes registros, são tocadas por diferentes
...
analítica destas notas e dos instrumentos que as tocam. As notas Mi3
e Si$\flat$2 são alternadas entre fagote e trompa para proporcionar um
...
pelo movimento das notas Sol e Lá.
...
Ré$\sharp$4 neste trecho. Finalmente a linha do fagote contém uma
